...
public class TitleScreen implements Screen {
	...
	public void update (float deltaTime) {
		if (Gdx.input.isTouched(0)) {
			...
			if (OverlapTester.pointInRectangle(scoresBounds, touchPos.x, touchPos.y)) {
				Assets.playSound(Assets.clickSound);
				game.setScreen(new ScoresScreen(game));
				return;
			}
			if (OverlapTester.pointInRectangle(tutorialBounds, touchPos.x, touchPos.y)) {
				Assets.playSound(Assets.clickSound);
				game.setScreen(new TutorialScreen(game));
				return;
			}
			if (OverlapTester.pointInRectangle(exitBounds, touchPos.x, touchPos.y)) {
				Assets.playSound(Assets.clickSound);
